    ...
    return (
        <>
            <div className="flex flex-col justify-center mb-4  lg:ml-32 ">
                ...
                <NavLink aria-disabled={disabled} onClick={() => handlePanelsShow()}
                         className="text-center text-[#367c2b] font-semibold hover:underline">{!usr.credentials ? 'Register' : 'Sign In'} with
                    a different email.</NavLink>
                    ...
                    <div>
                        <div className="flex flex-col mt-4">
                            ...
                            <input type="text" name="firstName" value={name.firstName} onChange={handleChange}
                                   ...
                                   className="w-[350px] border p-2 border-gray-600 rounded-md outline-none" required/>
                        </div>
                ...
                <div className={!passRem ? "flex mt-3" : "hidden"}>
                    ...
                    <p className=" text-[#c13833] ml-[8px] text-sm w-[325px]">Password must be at least 16 characters OR
                        at least 8 characters
                        including a number and a letter.</p>
                </div>
                <div className="flex my-3">
                    <input type={"checkbox"} disabled={disabled} checked={showPass}
                ...
                <button disabled={disabled}
                        ...
                        onClick={handleSubmit}>{usr.loading ?
                    <ClipLoader color="#36d7b7"/> : !usr.credentials ? 'Create Account' : 'Sign In'}
                </button>
                {usr.credentials ? <NavLink to="accounts/login/email-lookup"
                                            className="text-center text-[#367c2b] font-semibold hover:underline">Forgot
                    Password</NavLink> : ''}
